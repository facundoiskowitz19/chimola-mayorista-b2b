...
| `compra_rapida.py` | **Fase 2.** Helpers puros de compra rápida: parser de códigos pegados (SKU/EAN,cant) y armado de items. **Fase 12**: `excel_plantilla` (export del filtro actual, todas las páginas, con columna Cantidad) y `texto_desde_excel` (re-importar ese Excel editado desde «Pegar códigos»). La página vive en `app.py`. |
