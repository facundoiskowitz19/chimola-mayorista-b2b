...
| `odoo_export.py` | **Fase 12.** Export de un pedido en formato de importación masiva de Odoo, SOLO franquicias (titulares de PV): dos hojas `Sin talle` (Producto="Comodín", talle único) e `Indu` (una fila por talle). "Cliente" solo en la primera fila de cada hoja, sin totales ni filas extra. Nombre del cliente = `clientes_overrides.odoo_cliente` (Admin → cliente), fallback nombre Aleph. Botón en confirmación y en Mis pedidos (no en cancelados). |
| `compra_rapida.py` | **Fase 2.** Helpers puros de compra rápida: parser de códigos pegados (SKU/EAN,cant) y armado de items. **Fase 13**: `excel_plantilla` v2 (fórmulas de subtotal con valores cacheados, hoja «Pedido» con FILTER dinámico de lo elegido + totales con descuento del cliente e IVA, link público a la foto de cada variante, miniaturas JPEG embebidas opcionales — se generan bajo demanda, tope 600 filas) y `texto_desde_excel` multi-hoja. La tabla de la página es scroll infinito (sin paginado, fotos por URL pública del bucket, variantes con foto primero). La página es una sola vista sin pestañas: la sección «Pegar códigos» se eliminó (2026-08-31); `resolver_pegado` sigue vivo detrás del uploader de Excel. La página vive en `app.py`. |
